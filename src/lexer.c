  ...
  while (lexer->c != 0) {
    ...
    if (lexer->c == '=') {
      if (lexer_peek(lexer, 1) == '>') {
        ...
        scss_token_T* tok = lexer_advance_token(lexer, init_token(strdup("=="), TOKEN_EQUALS_EQUALS));
        lexer_advance(lexer);
        return tok;
      }
    }
    ...
    if (lexer->c == '-') {
      if (lexer_peek(lexer, 1) == '=') {
        scss_token_T* tok = lexer_advance_token(lexer, init_token(strdup("-="), TOKEN_MINUS_EQUALS));
        lexer_advance(lexer);
        return tok;
      } else if (lexer_peek(lexer, 1) == '-') {
...
scss_token_T* lexer_parse_id(lexer_T* lexer)
{
  ...
  while (isalnum(lexer->c) || isdigit(lexer->c) || lexer->c == '_' || lexer->c == '$' || lexer->c == '-') {
    str = str_append(&str, lexer->cstr);
    lexer_advance(lexer);
  }
